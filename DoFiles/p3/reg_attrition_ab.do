...
*Regression 2w
eststo : reg attrition_2w i.group , robust cluster(fecha_alta)
...
*Regression 2m
eststo : reg attrition_2m i.group, robust cluster(fecha_alta)
estadd scalar Erre=e(r2)
	forvalues i=1/2 {
		qui count if group==`i' & e(sample)
		local obs_`i'=r(N)
...
*Regression 2w2m
eststo : reg attrition_2w2m i.group, robust cluster(fecha_alta)
estadd scalar Erre=e(r2)
	forvalues i=1/2 {
		qui count if group==`i' & e(sample)
		local obs_`i'=r(N)
	...
	*************************
	esttab using "$directorio/Tables/reg_results/reg_attrition_ab.csv", se star(* 0.1 ** 0.05 *** 0.01) b(a2) ///
	scalars("Erre R-squared" "obs_per_gr Obs per group" "days_per_gr Days per group") replace 
