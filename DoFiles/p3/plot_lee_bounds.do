...
*COMPUTATION OF BOUNDS
local v=1
foreach vardep of varlist conflicto_arreglado hablo_con_abogado {
	foreach t in 2 3 b {
		preserve
		do "$directorio\DoFiles\lee_bounds.do" ///
				`vardep' `t' "no_condition" "" `v'
...
foreach t in 2 3 b {
	preserve
	do "$directorio\DoFiles\lee_bounds.do" ///
			cond_hablo_con_publico `t' hablo_con_abogado "" `v'
	restore
	local v=`v'+1
	}
...
foreach t in 2 3 b {
	preserve
	do "$directorio\DoFiles\lee_bounds.do" ///
			coyote `t' cond_hablo_con_publico hablo_con_abogado `v'
	restore
	local v=`v'+1
	}
...
twoway  (rcap ub lb k if inlist(k ,1,10,19, 28) ,   ///
		...
		, scheme(s2mono) graphregion(color(white)) ///	
		legend(order (1 "T1/T2" 2 "T1/T3" 3 "A/B") cols(3)  symysize(.1)) ///
		 title("Lee Bounds") subtitle("2W") ytitle("ATE") xtitle("") ///
		 xlabel(2 "Solved conflict"	 11 "Talked to Lawyer" 20 "Talked w/public" 29 "Inf Lawyer" )
graph export "$directorio/Figuras/lee_bounds_2w.pdf", replace 		 
...
twoway  (rcap miu mil k if inlist(k ,1,10,19, 28) ,   ///
		...
		, scheme(s2mono) graphregion(color(white)) ///	
		legend(order (1 "T1/T2" 2 "T1/T3" 3 "A/B") cols(3)  symysize(.1)) ///
		 title("Manski-Lee Bounds") subtitle("2W") ytitle("ATE") xtitle("") ///
		 xlabel(2 "Solved conflict"	 11 "Talked to Lawyer" 20 "Talked w/public" 29 "Inf Lawyer" )		 
graph export "$directorio/Figuras/manski_lee_bounds_2w.pdf", replace 		 
