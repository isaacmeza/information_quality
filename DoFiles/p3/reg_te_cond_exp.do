*TREATMENT EFFECTS REGRESSIONS CONDITIONING ON THOSE WHO ALREADY HAVE FORMED 
*EXPECTATIONS
...
*Conditioning on those who already have formed expectations
keep if !missing(cantidad_ganar) & ! missing(prob_ganar)
********************************************************************************
...
foreach var in `depvar'	{
	...
	*************************
	esttab using "$directorio/Tables/reg_results/reg_2w_main_arms_cond_exp.csv", se star(* 0.1 ** 0.05 *** 0.01) b(a2) ///
...
foreach var in `depvar'	{
	...
	*************************
	esttab using "$directorio/Tables/reg_results/reg_2m_main_arms_cond_exp.csv", se star(* 0.1 ** 0.05 *** 0.01) b(a2) ///
	scalars("Erre R-squared" "BVC BVC" "Source Source" "obs_per_gr Obs per group" "days_per_gr Days per group" "test_23 T2=T3") replace 
