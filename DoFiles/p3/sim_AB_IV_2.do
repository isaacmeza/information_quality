...
local p_baseline=0.17
*Probability of treatment
local p_treat=0.5
*Probability of attrition 
local p_attrition=0.85 
local diff_attrition=-0.05 
*Significance level
local signif=0.055
*Grid for beta_iv (ATT)
local beta_iv "0(0.05)0.7"
local length_iv=14
local sd_iv=0.3
*Grid for beta_fs (FS)
...
forvalues mu_x = `beta_fs' {
	...
	*Define matrices in which power is stored 
	...
	clear
	qui svmat pwr_`k'
	qui export delimited using "$directorio\Simulations\pwr_iv2_`k'.csv", replace novarnames
