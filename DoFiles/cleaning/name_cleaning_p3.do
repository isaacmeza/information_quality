...
replace despacho_actor = "PROCURADURIA" if tipo_abogado==3
...
gen despacho_office = despacho_actor
gsort gp_orden -despacho_actor
bysort gp_orden : replace despacho_actor = despacho_actor[_n-1]  if missing(despacho_actor)

*Group 'despachos' according to Levenshtein distance
strgroup despacho_office , gen(gp_despacho_office) threshold(.10) normalize(longer)

sort gp_despacho_office despacho_office
by gp_despacho_office : replace despacho_office = despacho_office[1]


*When office name is not known we impute name of lawyers
bysort gp_orden : gen repeats = _N
replace despacho_actor = abogados_orden if missing(despacho_actor) & repeats>=3


*Ignore "NO MENCIONA" and "PROCURADURIA" cases
replace gp_despacho = . if tipo_abogado==3

*Encodes the gp_despacho id that are essentially the same; later on we recode them
preserve
...
drop gp_despacho nombre_1 nombre_2 nombre_3 order_names1 order_names2 order_names3  despacho_office gp_despacho_office  m_1 m_2 m_3 m_4 m_5 m_6 m_7 unique  