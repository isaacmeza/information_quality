...
qui su survey_date
drop if inrange(survey_date,`r(min)',date("31/03/2019","DMY"))!=1  & (demanda_exact!=1)
...
qui putexcel I6=matrix(admin_survey) using "$directorio\Tables\reg_admin.xlsx", sheet("tab_admin_survey") modify
tab sue_fuzzy entablo_demanda, m matcell(admin_fuzzy)
qui putexcel I6=matrix(admin_fuzzy) using "$directorio\Tables\reg_admin.xlsx", sheet("tab_fuzzy_survey") modify
...
qui putexcel K5=matrix(r(matrix)) using "$directorio\Tables\reg_admin.xlsx", sheet("balance_admin_survey") modify
...
foreach var in `depvar'	{
	...
	*************************
	esttab using "$directorio/Tables/reg_results/reg_admin_main_arms.csv", se star(* 0.1 ** 0.05 *** 0.01) b(a2) ///
	scalars("Erre R-squared" "BVC BVC" "Source Source" "obs_per_gr Obs per group" "days_per_gr Days per group" "DepVarMean DepVarMean" "test_23 T2=T3") replace 
...
foreach var in `depvar'	{
	...
	*************************
	esttab using "$directorio/Tables/reg_results/reg_admin_main_arms_ab.csv", se star(* 0.1 ** 0.05 *** 0.01) b(a2) ///
